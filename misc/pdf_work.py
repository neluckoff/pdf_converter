...
def choose_funct(choosing, path, text, save_path):
    ...
    if choosing == "Get information about a PDF file":
        ...
        if choosing == "Convert PDF to DOCX":
            ...
            Button(new_app, text='Delete', command=lambda: pdf_delete_page(path, int(entry.get()),
                                                                           new_app, text, save_path))\
                .grid(row=0, column=2)
            new_app.mainloop()
            ...
            entry = Entry(new_app, textvariable=var1)
            entry.grid(row=0, column=1)
            Button(new_app, text='Select', command=lambda: pdf_select_page(path, int(entry.get()), new_app,
                                                                           text, save_path))\
                .grid(row=0, column=2)
            new_app.mainloop()
            ...
            entry = Entry(new_app, textvariable=var1)
            entry.grid(row=0, column=1)
            Button(new_app, text='Encrypt', command=lambda: encrypt_pdf(path, entry.get(),
                                                                        new_app, text, save_path)) \
                .grid(row=0, column=2)
            new_app.mainloop()
            ...
            entry = Entry(new_app, textvariable=var1)
            entry.grid(row=0, column=1)
            Button(new_app, text='Decrypt', command=lambda: decrypt_pdf(path, entry.get(),
                                                                        new_app, text, save_path)) \
                .grid(row=0, column=2)
            new_app.mainloop()
    else:
...
def pdf_to_docx(path, text, save_path):
    ...
    if Path(path).is_file() and (Path(path).suffix == ".pdf" or Path(path).suffix == ".PDF"):
        ...
        cv = Converter(path)
        print(save_path)
        with open(f'{save_path}/{docx_file}', 'wb') as out:
            cv.convert(docx_file, start=0, end=None, path=save_path)
        cv.close()
